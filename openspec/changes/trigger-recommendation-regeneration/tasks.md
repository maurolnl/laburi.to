...
- [ ] 1.1 Crear `internal/recommendation/triggers.go` con el tipo `Trigger`, que envuelve un
  ...
  `go build ./...`.
- [ ] 1.2 Implementar `EmployeeProfileCompleted(ctx, employeeID int32) error` y
...
- [ ] 1.3 Cubrir con un test que el error del `JobPublisher` se propaga sin envolver información
  extra, para que el borde de escritura pueda registrarlo tal cual.
- [ ] 1.4 Verificar que la dirección de dependencias se conserva:
  `go list -deps ./internal/recommendation | grep -E 'internal/(jobposition|employee)'` sin
  coincidencias.

## 2. Puerto angostado en `internal/jobposition`

- [ ] 2.1 Cambiar en `internal/jobposition/events.go` la firma de `JobPositionEventPublisher` a
...
- [ ] 2.2 Ajustar `CreateJobPosition` y `UpdateJobPosition` en `internal/jobposition/service.go`
  para notificar con `position.ID` después de persistir; verificar con `go build ./...`.
- [ ] 2.3 Actualizar el doble de `internal/jobposition/service_test.go` a la firma nueva y adaptar
  las aserciones para comparar identificadores; verificar con `go test ./internal/jobposition`.
- [ ] 2.4 Sumar tests de los escenarios del requisito modificado que la suite todavía no cubre:
...
- [ ] 2.5 Actualizar `internal/jobposition/doc.go` para describir la notificación por identificador.

## 3. Completitud del perfil en `internal/employee`

- [ ] 3.1 Agregar a `sql/queries/employees.sql` la consulta `IsEmployeeProfileComplete :one`, que
  ...
  sección tiene datos».
- [ ] 3.2 Ejecutar `sqlc generate` y revisar la salida en `internal/database`, sin editarla a mano;
  verificar con `go build ./...`.
- [ ] 3.3 Agregar `IsProfileComplete(ctx context.Context, employeeID int32) (bool, error)` a
...
- [ ] 3.4 Actualizar los dobles de `internal/employee/fakes_test.go` con el método nuevo, con un
...
- [ ] 4.1 Crear `internal/employee/events.go` con `EmployeeEventPublisher`
  ...
  `go build ./...`.
- [ ] 4.2 Inyectar el puerto en `employeeService` y en `NewService`/`BuildHandlers`
  (`internal/employee/handler.go`, `service.go`), manteniendo el orden
  `route -> handler -> service -> store`; verificar con `go build ./...`.
- [ ] 4.3 Implementar el disparo en las diez escrituras de perfil —`CreateEmployee`,
...
- [ ] 4.4 Cubrir con tests de servicio, usando productor falso, que cada una de esas escrituras
...
- [ ] 4.5 Cubrir con tests que ninguna de esas escrituras notifica cuando `IsProfileComplete`
  devuelve `false`, y que un fallo del repositorio no notifica ni consulta la completitud;
  verificar con `go test ./internal/employee`.
- [ ] 4.6 Cubrir con un test que un fallo del productor deja la escritura persistida y la operación
  resuelta sin error para el llamador; verificar con `go test ./internal/employee`.
- [ ] 4.7 Cubrir con un test que un fallo de `IsProfileComplete` no notifica y no convierte la
  escritura en un error, porque el cambio de dominio ya está confirmado; verificar con
  `go test ./internal/employee`.
- [ ] 4.8 Actualizar el comentario de paquete de `internal/employee` para declarar la notificación
  tras cada escritura que deja el perfil completo.

## 5. Cableado en `cmd`

- [ ] 5.1 En `cmd/api.go`, construir el `recommendation.Trigger` a partir de
  ...
  `go build ./...`.
- [ ] 5.2 Inyectar ese trigger en `employee.BuildHandlers` y en `jobposition.BuildHandlers`,
...
- [ ] 6.1 Agregar a `docs/recommendation-queue.md` una sección de disparadores: qué operaciones
...
- [ ] 6.2 Actualizar `CLAUDE.md` y `AGENTS.md` del backend: `internal/recommendation` ya tiene
  productor cableado y los disparadores dejaron de ser trabajo pendiente de LAB-34.
- [ ] 6.3 Ejecutar `gofmt -l .` sin salida, `go vet ./...` y `go test ./...`.
- [ ] 6.4 Comprobar explícitamente cada criterio de aceptación del ticket contra un test o una
