...
- [x] 1.1 Crear `internal/jobposition/factories_test.go` con `testJobPositionData`, defaults
...
- [x] 1.2 Garantizar datos independientes: clonar toda slice recibida por override y devuelta,
...
- [x] 1.3 Mover a las factories los dobles y helpers compartidos (`fakeJobPositionStore`,
...
- [x] 2.1 Migrar `repo_test.go` a las factories, reemplazando los literales de parámetros y
  filas por construcciones con overrides, sin cambiar assertions; verificar con
  `go test ./internal/jobposition -run Repository`.
- [x] 2.2 Migrar `service_test.go` y `handler_test.go` a las factories, incluido el cuerpo JSON
...
- [x] 3.1 Recorrer desde el borde HTTP los cinco valores de `required_experience` y los cuatro
...
- [x] 3.2 Cubrir los límites válidos de horas disponibles (`1` y `8`) junto a los inválidos
  (`0` y `9`); verificar el status esperado en alta y edición, y que el servicio no se alcanza
  cuando la validación falla.
- [x] 3.3 Cubrir recursos técnicos nulos, omitidos, vacíos y múltiples atravesando handler,
...
- [x] 4.1 Confirmar y, donde falte, completar los escenarios de usuario sin perfil de
...
- [x] 4.2 Cubrir la ausencia de reapertura a nivel HTTP: `PUT` y `DELETE` sobre un puesto
...
- [x] 4.3 Cubrir que la atribución del puesto se deriva siempre del JWT y no del cuerpo:
...
- [x] 5.1 Verificar publicación inmediata tras alta y edición con el publicador fake, ausencia
...
- [x] 5.2 Verificar que el puerto `JobPositionEventPublisher` se satisface con dobles y con
  `NoopEventPublisher`, y que ninguna prueba del paquete requiere SQS, red, credenciales,
  `DB_URL` ni variables de entorno.

## 6. Validación final

- [x] 6.1 Ejecutar `gofmt -l` sobre los archivos modificados, `go vet ./...` y `go test ./...`;
