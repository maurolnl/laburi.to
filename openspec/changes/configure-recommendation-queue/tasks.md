...
- [x] 1.1 Agregar `github.com/aws/aws-sdk-go-v2/service/sqs` con `go get` y verificar que
  queda como dependencia directa en `go.mod` y que `go build ./...` compila.
- [x] 1.2 Crear `internal/queue/doc.go` declarando el alcance —configuración y acceso, no
...
- [x] 2.1 Crear `internal/queue/errors.go` con `ErrQueueDisabled`, `ErrMissingQueueConfig` y
  ...
  inversa.
- [x] 2.2 Crear `internal/queue/config.go` con el tipo `Lookup func(string) (string, bool)`,
...
- [x] 2.3 Implementar en `LoadConfig` el cortocircuito de cola deshabilitada: sin
...
- [x] 2.4 Implementar la validación de los tres valores obligatorios con la cola habilitada
...
- [x] 2.5 Implementar los cuatro parámetros numéricos con su default y su rango según la
...
- [x] 2.6 Agregar el endpoint opcional `AWS_SQS_ENDPOINT_URL`; verificar con un test que
  ausente deja el campo vacío sin error y que presente lo conserva.
- [x] 2.7 Agregar un test que recorre todos los errores que `LoadConfig` puede producir y
...
- [x] 3.1 Crear `internal/queue/client.go` con el tipo `Message` (identificador, cuerpo,
...
- [x] 3.2 Documentar en el doc comment de `Client` que la entrega es al menos una vez, que
...
- [x] 3.3 Crear `internal/queue/disabled.go` con `Disabled` implementando `Client` y
...
- [x] 4.1 Crear `internal/queue/sqs.go` con la implementación real: construye la
...
- [x] 4.2 Implementar `Send`, `Receive`, `Delete` y `ExtendVisibility` mapeando desde y
...
- [x] 4.3 Agregar `queue.New(ctx, Config) (Client, error)` que devuelve `Disabled{}` cuando
...
- [x] 5.1 Crear `internal/queue/queuetest/fake.go` con un doble en memoria que implementa
...
- [x] 5.2 Implementar las garantías observables: un mensaje recibido queda invisible hasta
...
- [x] 5.3 Agregar al doble la posibilidad de forzar error en cada operación y de inspeccionar
  los mensajes enviados, que es lo que LAB-32 y LAB-33 necesitarán para sus tests;
  verificar con un test de cada capacidad.
- [x] 5.4 Verificar que el doble no entra al binario con
  `go list -deps ./cmd | grep queuetest` sin coincidencias.

## 6. Composición en cmd sin globals

- [x] 6.1 Extender `cmd/config.go` con el campo de configuración de cola dentro de
...
- [x] 6.2 Construir el cliente en `cmd/api.go` con `queue.New` y guardarlo como campo de
  `application`; verificar con `grep -rn "^var \|func init(" cmd/*.go internal/queue/` que
  no se introdujo ninguna global ni `init()`.
- [x] 6.3 Registrar en el arranque una única línea indicando si el transporte de
...
- [x] 6.4 Verificar que `cmd` no referencia `jobposition.NoopEventPublisher` de otra forma
...
- [x] 7.1 Agregar a `.env.example` las variables nuevas bajo una sección propia, todas sin
...
- [x] 7.2 Crear `docs/recommendation-queue.md` con la tabla de variables (obligatoriedad,
...
- [x] 7.3 Actualizar la sección de arquitectura de `CLAUDE.md` y `AGENTS.md` del backend
  ...
  sin diferencias.
- [x] 7.4 Verificar que no se modificó `.env` con `git status --short` y que `.env` no
  aparece en el diff del cambio.

## 8. Validación y cierre

- [x] 8.1 Ejecutar `gofmt -l internal/queue cmd` sin salida y `go vet ./...` sin hallazgos.
- [x] 8.2 Ejecutar `go test ./internal/queue/...` y luego `go test ./...`, ambos en verde,
  sin credenciales AWS en el entorno.
- [x] 8.3 Recorrer los cinco criterios de aceptación de LAB-31 y dejar constancia de con qué
  test o comprobación se satisface cada uno.
- [x] 8.4 Verificar que no se agregó observabilidad fuera de los logs mínimos: sin métricas,
