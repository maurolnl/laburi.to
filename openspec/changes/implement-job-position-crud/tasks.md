...
- [ ] 1.1 Crear `internal/jobposition/models.go` con `CreateJobPositionRequest`,
...
- [ ] 1.2 Crear `internal/jobposition/errors.go` con los sentinelas
...
- [ ] 1.3 Agregar a `internal/` un helper que responda los errores de validación como
...
- [ ] 2.1 Definir `JobPositionStore` en `internal/jobposition/store.go` con
...
- [ ] 2.2 Implementar `internal/jobposition/repo.go` sobre las queries sqlc de LAB-24,
...
- [ ] 2.3 Validar `timezone` en el repositorio contra `pg_timezone_names` replicando
...
- [ ] 3.1 Crear `internal/jobposition/events.go` con la interfaz
...
- [ ] 4.1 Implementar `internal/jobposition/service.go` con `NewService(store, publisher)`
...
- [ ] 4.2 Contrastar el `employerID` del path contra el empleador derivado del JWT en
...
- [ ] 4.3 Invocar `JobPositionPublished` solo tras un alta o edición persistida con
...
- [ ] 4.4 Garantizar que el listado devuelve siempre un slice no nulo y que get, update
  y delete sobre un puesto eliminado devuelven `ErrJobPositionNotFound`; verificar con
  tests de servicio sobre un store falso.

## 5. Handler y rutas

- [ ] 5.1 Implementar `internal/jobposition/handler.go` con `NewHandler`,
...
- [ ] 5.2 Decodificar el cuerpo rechazando JSON malformado y contenido extra tras el
...
- [ ] 5.3 Traducir los sentinelas a status: `403` para rol o ownership y para perfil de
...
- [ ] 5.4 Responder `201` con el puesto creado, `200` en listado, consulta y edición, y
...
- [ ] 6.1 Cablear en `cmd/api.go` el repositorio de puestos, el `NoopEventPublisher` y
  `jobposition.RegisterRoutes` dentro de `mountFeatureRoutes`; verificar con
  `go build ./...` y arrancando el servidor con `make build`.
- [ ] 6.2 Actualizar el `doc.go` de `internal/jobposition` para que describa el paquete
  ya completo en vez de anunciar que los handlers llegan después; verificar leyendo el
  archivo.

## 7. Validación y cierre

- [ ] 7.1 Ejecutar `gofmt -l .`, `go vet ./...` y `go test ./...` y dejar las tres
  salidas limpias.
- [ ] 7.2 Comprobar uno por uno los criterios de aceptación de LAB-25 contra los tests y
  ...
  servicios en verde.
- [ ] 7.3 Actualizar `../docs/create-job-position.md` reemplazando
...
- [ ] 8.1 Agregar las cinco rutas nuevas a la sección "Contrato HTTP actual" de
