...
- [x] 1.1 Crear `sql/schema/0007_recommendations.sql` con la tabla
...
- [x] 1.2 Agregar el check `recommendation_batches_status_check` con exactamente
...
- [x] 1.3 Agregar los índices únicos parciales
...
- [x] 1.4 Agregar el índice `recommendation_batches_employee_recent_idx` sobre
...
- [x] 1.5 Agregar en la misma migración la tabla `recommendations`:
      ...
      `score NUMERIC(7,4)` nullable y `created_at TIMESTAMPTZ NOT NULL DEFAULT now()`.
      Verificar que `score` no tiene `NOT NULL` ni `DEFAULT`.
- [x] 1.6 Agregar la constraint `recommendations_unique_pair_per_batch`
...
- [x] 1.7 Escribir la sección `-- +goose Down` que elimina índices y tablas en orden
      inverso, y verificar que no toca objetos de migraciones anteriores.
- [x] 1.8 Aplicar la migración y su rollback contra la base efímera y verificar que
      `up` seguido de `down` deja el esquema exactamente como estaba.

## 2. Infraestructura de test con PostgreSQL efímero

- [x] 2.1 Agregar `github.com/testcontainers/testcontainers-go` y su módulo de
...
- [x] 2.2 Crear `internal/testsupport/postgres.go` con un helper que levante el
...
- [x] 2.3 Implementar la degradación: si el entorno de contenedores no está disponible,
...
- [x] 2.4 Implementar el aislamiento entre tests: transacción con rollback por defecto y
...
- [x] 2.5 Verificar que el helper ignora cualquier `DB_URL` del entorno y que nunca
      aplica migraciones sobre una base preexistente, leyendo el código y comprobando
      que no consulta esa variable.

## 3. Acceso tipado

- [x] 3.1 Crear `sql/queries/recommendations.sql` con `CreateRecommendationBatch :one`,
...
- [x] 3.2 Implementar las dos queries de listado con `JOIN job_positions` y
...
- [x] 3.3 Implementar `GetCurrentBatch*` como el batch más reciente por `created_at`
      ...
      asume que coinciden.
- [x] 3.4 Ejecutar `sqlc generate` y verificar que `internal/database/` compila con
...
- [x] 4.1 Crear `internal/recommendation/models.go` con los tipos de dominio del batch,
...
- [x] 4.2 Crear `internal/recommendation/store.go` con la interfaz de consumidor y
...
- [x] 4.3 Implementar el reemplazo atómico en una única operación transaccional con
...
- [x] 4.4 Implementar la resolución del estado vigente y del conjunto vigente como una
      ...
      por separado.
- [x] 4.5 Crear `internal/recommendation/factories_test.go` con el patrón de opciones de
...
- [x] 5.1 Test de sujeto excluyente: verificar contra la base efímera que se aceptan los
      batches por empleado y por puesto, y que se rechazan los cuatro casos inválidos de
      la tarea 1.2.
- [x] 5.2 Test de ciclo de estados: verificar que los cuatro estados se aceptan, que un
      estado desconocido se rechaza, que la creación sin estado deja `pending` y que una
      transición refresca `updated_at`.
- [x] 5.3 Test de unicidad del batch en curso: verificar que un segundo batch `pending`
      ...
      simultáneos.
- [x] 5.4 Test de la relación many-to-many: varios empleados para un puesto, varios
...
- [x] 5.5 Test de puntaje: ausencia de puntaje, puntaje persistido sin pérdida de
      precisión, y puntaje cero distinguible de puntaje ausente al leerlo.
- [x] 5.6 Test de reemplazo atómico con commit real: verificar que al completar un batch
      el conjunto vigente pasa a ser el nuevo y los batches anteriores del sujeto
      desaparecen junto con sus recomendaciones.
- [x] 5.7 Test de fallo en el reemplazo: forzar un error en medio de la transacción
...
- [x] 5.8 Test de retención: tras varias ejecuciones sucesivas, verificar que el sujeto
      conserva a lo sumo el último `completed` y el último no completado.
- [x] 5.9 Test de estado vigente contra conjunto vigente: cubrir los cinco escenarios del
      spec, en particular que un batch `failed` posterior a un `completed` expone estado
      `failed` y conserva el conjunto anterior.
- [x] 5.10 Test de resultado vacío: un batch `completed` sin recomendaciones se
      distingue de uno `failed` por su estado y no por la cantidad de filas.
- [x] 5.11 Test de exclusión de puestos eliminados: eliminar lógicamente un puesto ya
      ...
      siguen apareciendo.
- [x] 5.12 Test de orden y paginación en ambos sentidos: orden por puntaje descendente,
...
- [x] 6.1 Ejecutar `gofmt -l .` y verificar que no lista archivos.
- [x] 6.2 Ejecutar `go vet ./...` y verificar que pasa.
- [x] 6.3 Ejecutar `go test ./...` con entorno de contenedores disponible y verificar que
      toda la suite pasa, incluidos los tests de integración.
- [x] 6.4 Ejecutar `go test ./...` con el entorno de contenedores detenido y verificar
      que los tests de integración se omiten con motivo y la suite no falla.
- [x] 6.5 Comprobar explícitamente cada criterio de aceptación de LAB-29 contra los tests
