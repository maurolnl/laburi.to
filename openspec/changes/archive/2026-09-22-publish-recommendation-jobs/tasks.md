...
- [x] 1.1 Ampliar `internal/recommendation/doc.go` para declarar que el paquete ahora también
  ...
  coincidencias.
- [x] 1.2 Crear `internal/recommendation/events.go` con `JobRequestedVersion` y el struct
...
- [x] 1.3 Implementar en `events.go` la construcción del evento a partir de un `Batch` y su
  ...
  dos tipos de sujeto.
- [x] 1.4 Cubrir con un test que el cuerpo serializado no contiene ningún atributo del
...
- [x] 1.5 Cubrir con un test que un cuerpo emitido se deserializa de vuelta al mismo evento y
...
- [x] 2.1 Crear `internal/recommendation/publisher.go` con la interfaz `JobPublisher`
  (`PublishJob(ctx, Subject) error`), documentando por qué recibe `Subject` y por qué no
  devuelve el batch; verificar con `go build ./...`.
- [x] 2.2 Implementar `NoopJobPublisher` en el mismo archivo, con la aserción de
...
- [x] 3.1 Implementar `QueueJobPublisher` en `internal/recommendation/publisher.go` con sus
...
- [x] 3.2 Implementar el camino feliz: validar el sujeto, abrir el batch `pending`, serializar
...
- [x] 3.3 Implementar el orden batch-antes-de-mensaje de forma observable: un test con un
  store que falla al crear debe comprobar que el fake no recibió ningún envío y que el error
  se propaga.
- [x] 3.4 Implementar el rechazo del sujeto inválido antes de tocar el store, devolviendo
  `ErrInvalidSubject`; verificar con un test que usa un `Subject` cero y comprueba que ni el
  store ni el fake fueron invocados.
- [x] 3.5 Implementar la deduplicación: ante `ErrBatchAlreadyInFlight` devolver `nil` sin
...
- [x] 4.1 Implementar el tratamiento del fallo de envío: transicionar el batch recién abierto
...
- [x] 4.2 Cubrir con un test que el transporte deshabilitado —`queue.Disabled`— produce el
  mismo desenlace: `errors.Is(err, queue.ErrQueueDisabled)` y batch en `failed`, sin que la
  emisión se reporte como exitosa.
- [x] 4.3 Implementar el caso en que la transición a `failed` también falla: devolver la causa
...
- [x] 4.4 Cubrir el reintento seguro con un test que, tras un fallo de publicación, emite una
...
- [x] 5.1 Cubrir con un test que la emisión no consulta ningún puntaje ni persiste ninguna
...
- [x] 5.2 Cubrir con un test que la emisión se resuelve sin esperar ningún cambio de estado
  del batch: tras `PublishJob`, el batch sigue en `pending` y la llamada ya retornó.

## 6. Documentación y cierre

- [x] 6.1 Actualizar `docs/recommendation-queue.md`: mover la publicación de "no implementado"
...
- [x] 6.2 Verificar el cambio completo con `gofmt -l .` sin salida, `go vet ./...` sin
  hallazgos y `go test ./...` en verde.
- [x] 6.3 Comprobar explícitamente cada criterio de aceptación del ticket contra un test o un
