...
- [ ] 1.1 Agregar a `internal/scoring/scorer.go` la interfaz opcional `Availability`
...
- [ ] 1.2 Implementar `Available` en `scoring.Unavailable` devolviendo `ErrScoringUnavailable`,
...
- [ ] 1.3 Cubrir con un test que un `Scorer` que no implementa `Availability` sigue cumpliendo
  el contrato: la aserción de tipo falla y no hay obligación de escribir el método.
- [ ] 1.4 Ampliar `internal/scoring/doc.go` para nombrar la comprobación previa como cuarto
  desenlace distinguible de los tres de `Result`.

## 2. Reclamo condicional y lectura del batch

- [ ] 2.1 Agregar a `sql/queries/recommendations.sql` la consulta `GetRecommendationBatch`
...
- [ ] 2.2 Agregar a `sql/queries/recommendations.sql` la consulta `ClaimRecommendationBatch`
...
- [ ] 2.3 Agregar `ErrBatchNotClaimable` a `internal/recommendation/errors.go`, distinto de
  `ErrBatchNotFound`, documentando que uno significa "ya cerrado" y el otro "no existe".
- [ ] 2.4 Agregar `GetBatch(ctx, batchID) (Batch, error)` y
  ...
  con `go build ./...`.
- [ ] 2.5 Cubrir con tests contra la base real de `internal/testsupport` que el reclamo
...
- [ ] 2.6 Cubrir con un test contra la base real que un reclamo fallido sobre un batch
...
- [ ] 2.7 Cubrir con un test que `GetBatch` sobre un identificador inexistente devuelve
  `ErrBatchNotFound` y no un batch cero.

## 3. Resolución de candidatos activos

- [ ] 3.1 Agregar a `sql/queries/recommendations.sql` la consulta que resuelve el perfil de
...
- [ ] 3.2 Agregar la consulta que lista todos los perfiles de scoring de empleados, con la
  misma forma que 3.1, para el sujeto puesto.
- [ ] 3.3 Agregar las consultas que resuelven un puesto vigente por identificador y que listan
...
- [ ] 3.4 Crear `internal/recommendation/candidates.go` con el puerto `CandidateSource`
  ...
  coincidencias.
- [ ] 3.5 Implementar `CandidateSource` en `RecommendationRepository`, traduciendo las filas a
...
- [ ] 3.6 Implementar la señal de sujeto inexistente: `ErrSubjectNotFound` cuando el empleado
  no existe o el puesto no existe o está eliminado lógicamente, distinguible de un universo
  vacío legítimo.
- [ ] 3.7 Cubrir con tests contra la base real, en ambos sentidos: universo completo, universo
  ...
  recursos técnicos.
- [ ] 3.8 Cubrir con un test que los pares devueltos superan `Pair.Validate()`, de modo que
  la traducción no produzca entradas que el contrato de scoring rechace.

## 4. Worker: ciclo de vida y configuración

- [ ] 4.1 Agregar a `internal/queue/config.go` la constante y la carga de
...
- [ ] 4.2 Crear `internal/recommendation/worker.go` con el struct del worker, sus dependencias
  —`RecommendationStore`, `CandidateSource`, `queue.Client`, `scoring.Scorer`— y su
  constructor; verificar con `go build ./...`.
- [ ] 4.3 Implementar el ciclo `Run(ctx)`: recibir lotes, procesar mensaje a mensaje y volver a
  ...
  nuevo después.
- [ ] 4.4 Implementar el tratamiento del error de recepción: registrar y reintentar sin
...
- [ ] 4.5 Implementar el procesamiento del mensaje en curso con un contexto desprendido de la
...
- [ ] 4.6 Implementar la extensión periódica de visibilidad mientras un mensaje se procesa, y
  verificar con un test que el fake recibió al menos una extensión en un procesamiento que
  supera el plazo configurado.
- [ ] 4.7 Cablear el worker en `cmd/api.go` y `cmd/main.go` detrás del flag, inyectando
...
- [ ] 5.1 Implementar la deserialización del cuerpo a `JobRequestedEvent` y el rechazo de un
...
- [ ] 5.2 Implementar el rechazo de una versión distinta de `JobRequestedVersion`, también
...
- [ ] 5.3 Implementar la resolución del batch con `GetBatch`: inexistente o terminal
  —`completed`, `failed`— reconoce el mensaje y termina sin trabajo; verificar con un test por
  cada uno de los tres casos.
- [ ] 5.4 Cubrir con un test de idempotencia extremo a extremo: procesar dos veces el mismo
...
- [ ] 5.5 Cubrir con un test que un batch que quedó en `processing` por un procesamiento
...
- [ ] 6.1 Implementar el orden del procesamiento definido en `design.md`: resolver candidatos
  ...
  batch en cada punto.
- [ ] 6.2 Implementar el camino de universo vacío: completar el batch con lista vacía sin
  consultar el scoring; verificar con un test por tipo de sujeto que usa un scorer doble que
  falla la prueba si se lo invoca.
- [ ] 6.3 Implementar el camino de scoring no disponible con candidatos: transicionar de
...
- [ ] 6.4 Implementar el camino completo: reclamar, invocar `ScoreAll`, descartar los resultados
  ...
  resultante.
- [ ] 6.5 Cubrir con un test que los resultados inelegibles no se persisten y que los elegibles
  sin puntaje sí, con puntaje nulo.
- [ ] 6.6 Cubrir con un test contra la base real que el conjunto vigente anterior sobrevive a un
  batch posterior fallido, y otro que un batch completado lo reemplaza entero sin dejar
  recomendaciones del anterior.
- [ ] 6.7 Cubrir con un test que un puesto eliminado lógicamente entre la emisión y el consumo
  no aparece en el conjunto generado para ningún empleado.
- [ ] 6.8 Implementar el camino de sujeto inexistente o eliminado: completar el batch con lista
  vacía y reconocer; verificar con un test por cada caso.

## 7. Worker: clasificación de errores y reconocimiento

- [ ] 7.1 Implementar la clasificación recuperable/terminal según la tabla de `design.md`, con
...
- [ ] 7.2 Implementar el no reconocimiento ante fallo de persistencia: el mensaje queda sin
  ...
  reloj.
- [ ] 7.3 Cubrir con un test que un fallo de `Delete` sobre un batch ya persistido no altera el
  estado persistido y que el redelivery se resuelve sin rehacer el trabajo.
- [ ] 7.4 Cubrir con un test que un mensaje envenenado —cuerpo ilegible— no impide que los
  mensajes siguientes del mismo lote se procesen.
- [ ] 7.5 Cubrir con un test que el agotamiento de reintentos es observable: tras N entregas sin
...
- [ ] 8.1 Cubrir con un test que el worker no conoce el algoritmo: sustituir el scorer por un
  doble que puntúa cambia el conjunto persistido y no requiere ningún cambio en el worker.
- [ ] 8.2 Verificar que el worker no menciona ningún tipo del SDK de SQS, con
  `grep -r "aws-sdk-go-v2" internal/recommendation` sin coincidencias.
- [ ] 8.3 Ampliar `internal/recommendation/doc.go` para declarar el alcance del consumidor, el
  orden de procesamiento y por qué el reclamo es condicional.
- [ ] 8.4 Actualizar `docs/recommendation-queue.md`: mover el consumo de "no implementado" a
  ...
  cuenta aparece.
- [ ] 8.5 Actualizar `.env.example` con el flag del worker, sin ningún valor real.

## 9. Cierre

- [ ] 9.1 Verificar el cambio completo con `gofmt -l .` sin salida, `go vet ./...` sin hallazgos
  y `go test ./...` en verde.
- [ ] 9.2 Comprobar explícitamente cada criterio de aceptación del ticket contra un test o un
