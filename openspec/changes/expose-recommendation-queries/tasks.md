...
- [ ] 1.1 Agregar a `sql/queries/recommendations.sql` las consultas `GetEmployeeOwner :one`
...
- [ ] 1.3 Declarar en `internal/recommendation/store.go` el puerto `SubjectOwnership` con
...
- [ ] 1.4 Implementar los dos métodos en `internal/recommendation/repo.go` sobre las consultas
  ...
  `go build ./...`.
- [ ] 1.5 Cubrir con un test de integración sobre `testsupport.PostgresTx` los cuatro escenarios
...
- [ ] 2.1 Agregar a `sql/queries/recommendations.sql` las consultas
...
- [ ] 2.2 Agregar el campo `Total int32` a `JobRecommendations` y `EmployeeRecommendations` en
  `internal/recommendation/store.go`, documentando que se resuelve contra el mismo batch
  completado que los items.
- [ ] 2.3 Ajustar `JobRecommendationsForEmployee` y `EmployeeRecommendationsForJobPosition` en
...
- [ ] 2.4 Extender los tests de `internal/recommendation/repo_test.go` con los cuatro escenarios
...
- [ ] 3.1 Crear en `internal/recommendation/models.go` el tipo de respuesta de paginación
  `PageInfo{Limit, Offset, Total int32}` con etiquetas JSON `limit`, `offset` y `total`.
- [ ] 3.2 Implementar en `internal/recommendation` el parseo de `limit` y `offset` desde la query
...
- [ ] 3.3 Cubrir el parseo con tests de tabla que afirmen cada caso del requisito «Validación de
...
- [ ] 4.1 Definir en `internal/recommendation/models.go` el tipo `QueryStatus` con los valores
...
- [ ] 4.2 Cubrir con un test que cada `BatchStatus` válido se traduce a su `QueryStatus`
  homónimo y que `ErrNoCurrentBatch` produce `none`; verificar con
  `go test ./internal/recommendation`.
- [ ] 4.3 Agregar a `internal/recommendation/errors.go` los sentinelas del borde:
  `ErrInvalidEmployeeID`, `ErrInvalidJobPositionID`, `ErrInvalidPagination`,
  `ErrRecommendationsForbidden` y `ErrEmployerProfileRequired`, con sus mensajes en inglés como
  el resto del paquete.

## 5. Service de consulta

- [ ] 5.1 Crear `internal/recommendation/service.go` con la interfaz `QueryService`
  ...
  `go build ./...`.
- [ ] 5.2 Implementar la autorización del sentido empleado: `user.AuthorizeProfileRole` contra
...
- [ ] 5.3 Implementar la autorización del sentido puesto: `user.AuthorizeProfileRole` contra
...
- [ ] 5.4 Traducir `ErrNoCurrentBatch` a una respuesta con estado `none`, items vacíos y total
  cero en ambos sentidos, en lugar de propagarlo como error.
- [ ] 5.5 Cubrir con tests de servicio y dobles todos los escenarios del requisito «Ownership
  ...
  `go test ./internal/recommendation`.
- [ ] 5.6 Cubrir con tests que los cinco estados llegan intactos al llamador y que `items` nunca
  es `nil`, incluidos el completado sin resultados y el fallido sobre un conjunto previo;
  verificar con `go test ./internal/recommendation`.

## 6. Handler y rutas

- [ ] 6.1 Crear `internal/recommendation/handler.go` con `QueryHandler`, `NewQueryHandler`,
...
- [ ] 6.2 Implementar los dos handlers: principal desde el contexto, identificador del path
...
- [ ] 6.3 Implementar la traducción de errores a HTTP: `401` sin principal, `400` para
  ...
  sujeto.
- [ ] 6.4 Cubrir con tests de handler sobre `httptest` y un service falso: los códigos de cada
...
- [ ] 6.5 Cubrir con un test de recorrido de páginas —dos páginas sucesivas del mismo conjunto—
...
- [ ] 7.1 En `cmd/api.go`, construir el handler de consulta sobre
  `recommendation.NewRepository(psqlDB)` y registrar sus rutas dentro de
  `mountFeatureRoutes`; verificar con `go build ./...` y `go vet ./...`.
- [ ] 7.2 Verificar que la dirección de dependencias se conserva:
  `go list -deps ./internal/recommendation | grep -E 'internal/(jobposition|employee)'` sin
  coincidencias.
- [ ] 7.3 Actualizar `internal/recommendation/doc.go` para declarar el borde de consulta, los
  cinco estados expuestos y la regla de ownership.

## 8. Documentación y validación final

- [ ] 8.1 Agregar a `docs/recommendation-queue.md` una sección de consulta: las dos rutas, el
...
- [ ] 8.2 Reescribir `../docs/employee-searching-for-position.md` y
...
- [ ] 8.3 Actualizar `CLAUDE.md` y `AGENTS.md` del backend: las consultas de recomendaciones
  dejan de ser trabajo pendiente de LAB-35 y pasan a ser endpoints implementados.
- [ ] 8.4 Ejecutar `gofmt -l .` sin salida, `go vet ./...` y `go test ./...`.
- [ ] 8.5 Comprobar explícitamente cada criterio de aceptación del ticket contra un test o una
